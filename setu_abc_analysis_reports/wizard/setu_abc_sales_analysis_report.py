...
class SetuABCSalesAnalysisReport(models.TransientModel):
    ...
    # warehouses_ids = fields.Many2many("stock.warehouse", 'abc_sale_analysis_report_warehouse_rel',
    #                                   'abc_sale_analysis_report_id',
    #                                   'warehouses_id',
    #                                   string="Warehouses",
    ...
    def set_column_width(self, workbook, worksheet):
        ...
        worksheet.set_column(2, 8, 16)

    def set_format(self, workbook, wb_format):
        """
            Author: Kinnari Tank | Date: 11/10/24 | Task: 1000
            Purpose: This method will use for prepare format for title in xls worksheet
        """
    ...
    def set_report_title(self, workbook, worksheet):
        ...
        worksheet.merge_range(0, 0, 1, 8, "ABC Sales Analysis Report", wb_format)
    ...
    def get_abc_sales_analysis_report_data(self):
        ...
        query = """
                Select * from get_abc_sales_analysis_data('%s','%s','%s','%s','%s','%s', '%s')
    ...
    def prepare_data_to_write(self, sales_data={}):
        """
        Author: Kinnari Tank | Date: 11/10/24 | Task: 1000
    ...
    def download_report(self):
        ...
        sales_data = self.get_abc_sales_analysis_report_data()
        warehouse_wise_analysis_data = self.prepare_data_to_write(sales_data=sales_data)
        if not warehouse_wise_analysis_data:
            return False
        workbook = self.create_excel_workbook(file_pointer)
        for sales_data_key, sales_data_value in warehouse_wise_analysis_data.items():
            sheet_name = sales_data_key[1]
            wb_worksheet = self.create_excel_worksheet(workbook, sheet_name)
            row_no = 5
            self.write_report_data_header(workbook, wb_worksheet, row_no)
            for abc_data_key, abc_data_value in sales_data_value.items():
                row_no = row_no + 1
        ...
        return {
            ...
            'url': '/web/binary/setu_abc_download_document?model=setu.abc.sales.analysis.report&field=stock_file_data&id=%s&filename=%s' % (
                self.id, file_name),
            'target': 'self',
        }
    ...
    def download_report_in_listview(self):
        ...
        """
        sales_data = self.get_abc_sales_analysis_report_data()
        for abc_data_value in sales_data:
        ...
        list_view_id = self.env.ref('setu_abc_analysis_reports.setu_abc_sales_analysis_bi_report_list').id
        ...
        return {
            'name': _('ABC Sales Analysis ({})'.format(report_date)),
    ...
    def write_report_data_header(self, workbook, worksheet, row):
        ...
        odd_normal_center_format.set_text_wrap()
        ...
        worksheet.write(row, 4, 'Total Sales', odd_normal_right_format)
        worksheet.write(row, 5, 'Total Sales Amount', even_normal_right_format)
        worksheet.write(row, 6, 'Total Orders', odd_normal_right_format)
        worksheet.write(row, 7, 'Total Sales Amount (%)', even_normal_right_format)
        # worksheet.write(row, 5, 'Cum. Sales Amount (%)', even_normal_right_format)
        worksheet.write(row, 8, 'ABC Classification', odd_normal_right_format)
        return worksheet

    def write_data_to_worksheet(self, workbook, worksheet, data, row):
        """
            Author: Kinnari Tank | Date: 11/10/24 | Task: 1000
            Purpose: This method will use for add records in xls worksheet with appropriate format
        """
        ...
        worksheet.write(row, 4, data.get('sales_qty', ''), odd_normal_right_format)
        worksheet.write(row, 5, data.get('sales_amount', ''), even_normal_right_format)
        worksheet.write(row, 6, data.get('total_orders',''), odd_normal_right_format)
        worksheet.write(row, 7, data.get('sales_amount_per', ''), even_normal_right_format)
        # worksheet.write(row, 5, data.get('cum_sales_amount_per', ''), even_normal_right_format)
        worksheet.write(row, 8, data.get('analysis_category', ''), odd_normal_center_format)
        return worksheet
